<template>
    <div class="listing-summary-group" data-offset=''>
        <h1>Places in {{country}}</h1>
        <div class="listing-carousel">
            <div class="controls">
                <carousel-control dir="left" v-on:change-image="change" :style="leftArrowStyle"></carousel-control><!-- change-image is an event that the carousel-control component emits -->
...
<script>
    ...
    export default {
        ...
        methods: {
            change(directionValue){ //note: -1 = left, 1 = right
                let newVal = this.offset + parseInt(directionValue);
                if (newVal >= 0 && newVal <= this.listings.length - rowSize) {
                    this.offset = newVal;
                }
                console.log("change image"); console.log(directionValue);
            }
        },
        computed: {
            style() {
